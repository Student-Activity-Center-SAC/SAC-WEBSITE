      ...
      <section style={{ background: '#faf6f1', paddingTop: '92px', paddingBottom: '72px' }}>
        <div className="w-full px-6 sm:px-12 xl:px-20">
          ...
          <p className="text-lg mb-10" style={{ color: '#71717A', maxWidth: '56ch' }}>
            24+ clubs across {totalDomains} domains. Every passion, every ambition — there's a club for you at KL SAC.
          </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-0" style={{ borderTop: '1px solid #F0F0F0' }}>
                  {domainClubs.map(club => (
                    <Link
                      key={club.slug}
                      prefetch={false}
                      href={(club.slug === 'acic-tif' || club.slug === 'acic-tbi') ? 'https://www.acickl.in/' : `/clubs/${club.slug}`}
                      target={(club.slug === 'acic-tif' || club.slug === 'acic-tbi') ? '_blank' : undefined}
                      rel={(club.slug === 'acic-tif' || club.slug === 'acic-tbi') ? 'noopener noreferrer' : undefined}
